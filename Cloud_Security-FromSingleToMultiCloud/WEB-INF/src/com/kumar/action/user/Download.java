...
public class Download extends HttpServlet
{
	...
	public void service(HttpServletRequest req,HttpServletResponse res)throws IOException
	{
		...
		String username = "";
		  ...
		  System.out.println("while download current username>>>>>>>>>>>>>>>>"+username);
		  fileName = (String) req.getParameter("fname");
		  fileType = (String) req.getParameter("ftype");
		  uId = req.getParameter("uid");
		  uploadId = Integer.parseInt(uId);
		 // int muid=userDAO.getuserid(Integer.parseInt(username));
	    	userid = userDAO.getID(username);
		  System.out.println("muid as userid>>>>>>>>>>>>>>>>>>>>>>>>>>>"+userid);
		  
		 System.out.println("uploaded file id is >>>>>>>>>>>>>>>>> "+uploadId );
				 ...
				 if(flag)
		            {
			            try 
			            {
		                   /* Adding the download Transaction details (Start)*/
			            	...
			            	File file = new File("D:\\Downloads\\Multi-Cloud\\"+fileName);
